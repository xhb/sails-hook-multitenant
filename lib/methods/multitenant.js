...
module.exports = function multitenant(req, model, self, sails, schema, WLModel) {
    ...
    return new Promise(async (resolve,reject) => {
        ...
        if (req && req.constructor.name === 'IncomingMessage') {
            ...
            // console.log('全部datastore: ', _datasource.collection)
            if(_datasource.search(config.identity)) {
                _self.datastore = config.identity;
                resolve(_self);
            } else {
                if(d.isDatasource(config) && config.identity) {
                    var _newdatasource = new d.datasource(config.host, config.port, config.schema, config.adapter, config.user, config.password, config.database, config.identity, config.url);
                    await createDatasource(model, _newdatasource, config.identity, schema, function(err, ok){
                        if (err){
                            // console.log('注册datastore失败: ', err)
                            throw new Error('注册datastore失败');
                        } else {
                            // console.log('注册datastore成功: ', ok)
                            _self.datastore = config.identity;
                            resolve(_self);
                        }
                    });
