...
module.exports = function(sails, SchemaMap, config, identity) {
    return new Promise(async (resolve,reject) => {
        ...
        const d_config = new d.datasource(config.host, config.port, config.schema, config.adapter, config.user, config.password, config.database, config.identity, config.url);
