module.exports.datasource = function DataStoreConfig(host, port, schema, adapter, user, password, database, identity, url) {
  ...
  // constructor
  function _constructor(host, port, schema, adapter, user, password, database, identity, url) {
  ...
  this.validate = function () {
    ...
    if (_self.schema === undefined) throw new Error('The datasource object not specify the schema propertie');
  ...
  // run the constructor
  _constructor(host, port, schema, adapter, user, password, database, identity, url);
}

module.exports.isDatasource = function (obj) {
  if (!obj.host) return false;
  if (!obj.port) return false;
  if (obj.schema === undefined) return false;
